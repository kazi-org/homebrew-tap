# Download strategy for release assets held in a private GitHub repository.
#
# Homebrew's HOMEBREW_GITHUB_API_TOKEN authenticates API calls but not asset
# downloads, so a private repository's release asset 404s under the default
# strategy. This resolves the asset through the API and fetches it with the
# token attached.
#
...
class GitHubPrivateRepositoryReleaseDownloadStrategy < GitHubPrivateRepositoryDownloadStrategy
  ...
  def fetch_release_metadata
    GitHub::API.open_rest("https://api.github.com/repos/#{@owner}/#{@repo}/releases/tags/#{@tag}")
  end
end
